...
void main() {
  group('computeMonthlySavings', () {
    test(
      'a category not present in the map is excluded, not assumed to be zero spend',
      () {
    ...
    test(
      'spending more than the benchmark contributes zero, never negative',
      () {
    ...
    test('dining out and snacks & drinks share one combined benchmark', () {
      final combined = computeMonthlySavings({
        'Dining Out': 100.0,
        'Snacks & Drinks': 50.0,
      });
    ...
    test(
      'zero spend in a tracked category counts as the full benchmark saved',
      () {
    ...
    test(
      'categories with no reliable benchmark (Taxi, Subscriptions) never contribute',
      () {
