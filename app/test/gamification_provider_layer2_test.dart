...
void main() {
  ...
  group('complete-log days', () {
    ...
    test('marking is idempotent and reflected by isCompleteLogDay', () async {
      ...
      await gamification.markCompleteLogDay(
        day,
      ); // second call should not throw or duplicate
  ...
  group('month-boundary category challenge evaluation', () {
    test(
      'first-ever call evaluates nothing (avoids a spurious retroactive win)',
      () async {
    ...
    test(
      'after seeding a prior-month baseline, evaluates last month and detects a win',
      () async {
        ...
        final wins = await gamification.evaluateMonthBoundaryIfNeeded();
        final result = gamification.categoryChallengeResults.firstWhere(
        ...
        expect(
          ...
          isTrue,
          reason:
              'zero spend in the un-logged previous month should count as a win',
        );
    ...
    test(
      'calling again within the same month is a no-op (idempotent)',
      () async {
        final now = DateTime.now();
        final settings = Hive.box(HiveBoxes.settings);
        await settings.put(
        ...
        expect(
          secondRun,
          isEmpty,
          reason: 'already evaluated this month, should not re-fire',
        );
    ...
    test(
      'categories with monthlyLimit <= 0 are skipped (no challenge set)',
      () async {
        final now = DateTime.now();
        final settings = Hive.box(HiveBoxes.settings);
        await settings.put(
  ...
  group('monthly savings recap (evaluated alongside the month boundary)', () {
    test(
      'a positive-savings month arms pendingMonthlySavingsCelebration',
      () async {
        final now = DateTime.now();
        final settings = Hive.box(HiveBoxes.settings);
        await settings.put(
    ...
    test(
      'calling again within the same month does not re-evaluate savings',
      () async {
        final now = DateTime.now();
        final settings = Hive.box(HiveBoxes.settings);
        await settings.put(
    ...
    test(
      'a category with no matching benchmark never produces a pending celebration on its own',
      () async {
        final now = DateTime.now();
        final settings = Hive.box(HiveBoxes.settings);
        await settings.put(
    ...
    test(
      'monthlySavingsHistory surfaces every evaluated month, including \$0 ones',
      () async {
        final now = DateTime.now();
        final settings = Hive.box(HiveBoxes.settings);
        await settings.put(
